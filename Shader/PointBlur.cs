...
public class PointBlur : MonoBehaviour
{
    ...
    private void Start()
    {
        //初始化振幅贴图（把waveform曲线初始化到gradTexture上面）
    ...
    private void Update()
    {
        ...
        if (am.blurEffect)
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                if (Spark != null)
                    clone = Instantiate(Spark, new Vector3(hit.point.x, hit.point.y, 0), Quaternion.identity);
                BlurCenter = Camera.main.WorldToScreenPoint(/*new Vector3(hit.point.x, hit.point.y, 0)*/pos.position);
